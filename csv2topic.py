"""
The following script is excerpted and adapted from TopicVec, the source code 
for "Generative Topic Embedding: a Continuous Representation of Documents" 
(ACL 2016) by Shaohua Li, Tat-Seng Chua, Jun Zhu and Chunyan Miao.
The full implementation is here: https://github.com/askerlee/topicvec
"""
import numpy as np
import getopt
import sys
...
customStopwords = "city council agenda minutes committee commission san francisco milpitas meeting authorities cupertino santa clara alameda san jose oakland mtc hercules mountain view south san francisco sunnyvale burlingame hayward leandro stockton mateo city exhibit report attachment recommendation district ordinance code supervisors councilmember supervisor manager seconding authorizing approving"

config = dict(  csv_filenames = ["55word_doc_text.csv"],
                short_name = None,
                unigramFilename = "vectors&corpus/top1grams-wiki.txt",
                word_vec_file = "vectors&corpus/25000-180000-500-BLK-8.0.vec",
                # word_vec_file = "vectors&corpus/word2vec.vec",
                K = 2000,
                N0 = 500,
                max_l = 5,
                init_l = 1,
...
def getOptions():
    ...
    try:
        ...
        basename = os.path.basename(config['csv_filenames'][0])
...
def main():
    ...
    if 'evalKmeans' in config and config['evalKmeans']:
        topicvec.kmeans()
        topicvec.printTopWordsInTopics(None, True)
        exit(0)
    ...
    # save_matrix_as_text( basetrunk.encode() + "-em%d-best.topic.vec".encode() %best_it, "topic".encode(), best_T  )

    if best_last_Ts[1]:
        last_it, last_T, last_loglike = best_last_Ts[1]
        save_matrix_as_text( basetrunk + "-em%d-last.topic.vec" %last_it, "topic", last_T  )
        # save_matrix_as_text( basetrunk.encode() + "-em%d-last.topic.vec".encode() %last_it, "topic".encode(), last_T  )
