...
class topicvecDir:
    def __init__(self, **kwargs):
        ...
        if os.path.isfile(embedding_npyfile):
            ...
            embedding_arrays = np.load(embedding_npyfile, allow_pickle=True, encoding='bytes')
        ...
        for wid, w in enumerate(self.vocab):
            ...
            if w.decode() not in self.vocab_dict:
                oovcount += 1
                u2.append(unigram_oov_prior)
            else:
                # u2.append( self.vocab_dict[w][2] )
                # vocab_dict2[w] = wid
                u2.append( self.vocab_dict[w.decode()][2] )
                vocab_dict2[w.decode()] = wid
    ...
    def printTopWordsInTopics( self, docs_theta, outputToScreen=False ):
        ...
        # wids2 = self.wid2freq.keys() # Word IDs. len(wids2) is the number of unique words
        wids2 = np.array(list(self.wid2freq.keys()))
        # self. V is a numpy array of all of the embeddings of words in the dictionary
        ...
        row_wordOccur = np.array( list(self.wid2freq.values()) )
        ...
        # AD: Initialize outer list to build 'list of list' of words in each topic
        # e.g., [['hat', 'hair', 'head'], ['red', 'blue', 'green']]
        topics = []
        ...
        for k in selTids:
            ...
            for rowID in rowID_sorted[:self.topW]:
                ...
                # Append each word in topic to words
                words.append(self.vocab[wid].decode())
            ...
            wid_sorted = sorted( range(self.Mstep_sample_topwords), 
            ...
            for wid in wid_sorted[:self.topW]:
                ...
                # Append each word in topic to words
                if self.vocab[wid] not in words:
                    words.append(self.vocab[wid].decode())

            # After both for loops end, add to topics list
            topics.append(words)
    ...
    def docSentences2wids( self, docs_wordsInSentences ):
        ...
        for wid, freq in wid_freqs[:30]:
            line += "%s(%d): %d " %( self.vocab[wid].decode(), wid, freq )
        out1(line)
    ...
    def setDocs( self, docs_wordsInSentences, docs_name ):
        ...
        for wid, freq in enumerate(self.wid2freq):
