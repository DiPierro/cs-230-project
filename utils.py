...
np.seterr(all="raise")
np.set_printoptions(suppress=True, threshold=np.sys.maxsize, precision=3)
...
def save_matrix_as_text( filename, rowTypeName, T, *extraCols, **kwargs ):
    ...
    FMAT.write( "%d %d %d\n" %( K, N, extraColNum ) )
    # FMAT.write( "%d %d %d\n".encode() %( K, N, extraColNum ) )
    
    for i in range(K):
        # if rowNames is provided, print the corresponding row name at the beginning of each line
        # line = "".encode()
        line = ""
        for j in range(extraColNum):
            # col = str( extraCols[j][i] ).encode()
            col = str( extraCols[j][i] )
            line += col + colSep
        line += "%.5f" %T[i,0]
        # line += "%.5f".encode() %T[i,0]
            
        
        for j in range(1, N):
            line += " %.5f" %T[i,j]
            # line += " %.5f".encode() %T[i,j]
        FMAT.write("%s\n" %line)
        # FMAT.write("%s\n".encode() %line)
